...
namespace Outracks.Fuse.Theming.Themes
{
	internal class OriginalDarkTheme : ITheme
	{
		...
		public Color Active
		{
			get { return Color.FromRgb(0x45c8e6); }
		}

		public Brush ActiveHover
		{
			get { return Color.FromRgb(0x3DB2CC); }
		}

		public Brush Purple
		{
			get { return Color.FromRgb(0xb066ff); }
		}
		...
		{
			readonly ITheme _sourceTheme;
			readonly ITheme _targetTheme;

			public ColorMap(ITheme sourceTheme, ITheme targetTheme)
			{
				_sourceTheme = sourceTheme;
				_targetTheme = targetTheme;
			}

			public Color Map(Color color)
			{
				if (color == _sourceTheme.IconPrimary)
					return _targetTheme.IconPrimary;
				if (color == _sourceTheme.IconSecondary)
					return _targetTheme.IconSecondary;
				return color;
			}
		}

		static readonly ColorMap _colorMap = new ColorMap(new OriginalLightTheme(), new OriginalDarkTheme());

		public IObservable<IColorMap> IconColorMap { get { return Observable.Return(_colorMap); } }
	}
}