//gcc -m64 monostub.m -o monostub -framework AppKit -mmacosx-version-min=10.6
...
int main (int argc, char **argv)
{
	...
	NSString *binDir = [[NSString alloc] initWithUTF8String: "Contents/MonoBundle"];
	...
	NSArray *comps = [NSArray arrayWithObjects:[exePath stringByDeletingLastPathComponent], @"..", @".mono_root", nil];
	NSString *monoFile = [NSString pathWithComponents:comps];
	NSURL *URL = [NSURL fileURLWithPath:monoFile];
	NSString *monoRoot = [[NSString alloc]
	...
	if([monoRoot characterAtIndex:0] == '~' || [monoRoot characterAtIndex:0] == '/')
		isAbsolutePath = true;

	if(!isAbsolutePath) {
		NSArray *pieces = [NSArray arrayWithObjects:[monoFile stringByDeletingLastPathComponent], monoRoot, nil];
		monoRoot = [NSString pathWithComponents:pieces];
	}
