...
function install_or_update
{
	...
	curl -s -L -o temp_package --header "If-None-Match: $LOCAL_ETAG" "https://go.fusetools.com/latest-sublime-plugin-2" --dump-header header_dump >/dev/null
	if [[ $status -ne 0 ]]; then
...
function check_for_update
{
	...
	curl -s -L --header "If-None-Match: $LOCAL_ETAG" "https://go.fusetools.com/latest-sublime-plugin-2" --dump-header header_dump >/dev/null
	if [[ $status -ne 0 ]]; then
