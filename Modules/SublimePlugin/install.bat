...
for /f "delims=" %%A in ('call .\tools\curl.exe -I -s -L --header "If-None-Match: %fuse_etag%" "https://raw.githubusercontent.com/fuse-open/Fuse.SublimePlugin/master/releases/FuseSublimePlugin_1_5_0.zip"') do call :checkHeader %%A %%B
IF %ERRORLEVEL% NEQ 0 (
	ECHO Download failed!
	GOTO error
	)
call .\tools\curl.exe -s -L -o ^"%fuse_package_file%^" "https://raw.githubusercontent.com/fuse-open/Fuse.SublimePlugin/master/releases/FuseSublimePlugin_1_5_0.zip"
IF %ERRORLEVEL% NEQ 0 (
	ECHO Download failed!
	GOTO error
	)
...
for /f "delims=" %%A in ('call .\tools\curl.exe -I -s -L --header "If-None-Match: %fuse_etag%" "https://raw.githubusercontent.com/fuse-open/Fuse.SublimePlugin/master/releases/FuseSublimePlugin_1_5_0.zip"') do call :checkHeaderUpdate %%A %%B
